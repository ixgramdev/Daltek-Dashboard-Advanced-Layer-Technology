...
function addWidget(frm, grid, widgetData, pos) {
    ...
    node.innerHTML = `
        <div class="grid-stack-item-content" style="padding:0;height:100%;box-sizing:border-box;position:relative;">
            <div style="margin:3px;padding:8px;background:${widgetData.options.color};border-radius:6px;height:calc(100% - 6px);box-sizing:border-box;display:flex;flex-direction:column;justify-content:space-between;position:relative;overflow:hidden;">
                <div style="display:flex;justify-content:space-between;align-items:center;position:relative;z-index:2;">
                    <h5 style="font-size:11px;color:white;margin:0;font-weight:600;">${widgetData.options.title}</h5>
                ...
                <span style="font-size:20px;color:white;display:block;text-align:center;font-weight:bold;position:relative;z-index:2;">${widgetData.options.number || 0}</span>
...
function renderWidgets(frm, grid) {
    ...
    widgets.forEach((widget) => {
        ...
        node.innerHTML = `
            <div class="grid-stack-item-content" style="padding:0;height:100%;box-sizing:border-box;position:relative;">
                <div style="margin:3px;padding:8px;background:${widget.properties.color};border-radius:6px;height:calc(100% - 6px);box-sizing:border-box;display:flex;flex-direction:column;justify-content:space-between;position:relative;overflow:hidden;">
                    <div style="display:flex;justify-content:space-between;align-items:center;position:relative;z-index:2;">
                        <h5 style="font-size:11px;color:white;margin:0;font-weight:600;">${widget.properties.title}</h5>
                        <button style="border:none;border-radius:50%;width:18px;height:18px;background:rgba(255,255,255,0.2);color:white;cursor:pointer;font-size:10px;">⚙</button>
                    </div>
                    <span style="font-size:20px;color:white;display:block;text-align:center;font-weight:bold;position:relative;z-index:2;">${widget.properties.number || 0}</span>
                    <div style="position:absolute;bottom:0;right:0;width:15px;height:15px;background:rgba(255,255,255,0.1);cursor:se-resize;z-index:3;border-radius:3px 0 6px 0;"></div>
                </div>
            </div>
        `;
